...
/**
 * Class for storing and manipulating the meeting probabilities for the RandomFor
 * router module.
 */
public class MeetingProbabilitySet {
    ...
    private Map<Connection,Double> timeOfConnections;
	...
	/*public MeetingProbabilitySet(int maxSetSize, double alpha) {
        ...
        timeOfConnections=new HashMap<>();
	}*/

	public MeetingProbabilitySet(int maxSetSize, Map<Connection,Double> timeOfConnections) {
		//this.alpha = alpha;
		timeOfConnections=new HashMap<>();
		this.timeOfConnections=timeOfConnections;
		this.probs = new HashMap<Integer, Double>();
		if (maxSetSize == INFINITE_SET_SIZE || maxSetSize < 1) {
			this.probs = new HashMap<Integer, Double>();
			this.maxSetSize = INFINITE_SET_SIZE;
	 ...
	 */
	public void updateMeetingProbFor(Integer index,DTNHost host, DTNHost otherHost) {
		...
		if(connections!=null) {
			for (Connection con : connections) {
				if (con.getOtherNode(host) == otherHost) {
					if (timeOfConnections.get(con) != null) {
						encounterPossibility = timeOfConnections.get(con);
					}
				}
			}
		}
		
		double newValue = getProbFor(index) + encounterPossibility;//Todo
	...
	public MeetingProbabilitySet replicate() {
		MeetingProbabilitySet replica = new MeetingProbabilitySet(
				this.maxSetSize, timeOfConnections);
